use iron_adapter::{IronJanitor, Janitor};
...
fn test_bridge_separation_of_powers() {
    ...
    let json_content = fs::read_to_string(&path).expect("Should be able to read mock file");
    ...
    assert!(
        initial_validation.is_err(),
        "Truth layer should have rejected dirty data by default"
    );
    ...
    match df_result {
        Ok(df) => {
            ...
            if let Some(v) = val_r1 {
                assert!(
                    (v - 1250.5).abs() < 0.001,
                    "Unit extraction failed. Expected 1250.5, got {}",
                    v
                );
            ...
            assert_eq!(
                ghost_series.null_count(),
                ghost_series.len(),
                "Ghost column should be silenced to all nulls"
            );
