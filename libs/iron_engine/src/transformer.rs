...
fn build_series(table: &TableTruth, col_def: &ColumnDef) -> Result<Series> {
    ...
    match &col_def.dtype {
        IronDataType::Int => {
            let values: Vec<Option<i64>> = table.rows
                .iter()
                .map(|row| {
                    row.cells.get(col_idx)
                        .and_then(|cell| cell.value.as_int())
                })
                .collect();
            Ok(Series::new((&col_def.name).into(), &values))
        }
        IronDataType::Float64 => {
            let values: Vec<Option<f64>> = table.rows
                .iter()
                .map(|row| {
                    row.cells.get(col_idx)
        ...
        IronDataType::Date => {
            ...
            Err(EngineError::InvalidData("Date type not yet implemented".to_string()))
        }
    }
}
...
mod tests {
    ...
    fn test_to_dataframe_exact_schema_match() {
        ...
        assert!(matches!(df.column("amount").unwrap().dtype(), polars::prelude::DataType::Float64));
        assert!(matches!(df.column("description").unwrap().dtype(), polars::prelude::DataType::String));
    ...
    fn test_to_dataframe_reject_schema_mismatch() {
        ...
        assert!(result.is_err(), "Should reject table with mismatched cell count");
    }
    ...
    fn test_dataframe_deterministic() {
        ...
        assert_eq!(df1.get_column_names(), df2.get_column_names(), "Column names must be identical");
