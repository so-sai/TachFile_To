...
use iron_table::{TableSchema, ColumnDef, DataType, TableRow, TableCell, CellValue, BoundingBox, ExtractionMeta, EncodingStatus};
...
fn mock_project_truth() -> ProjectTruth {
    ProjectTruth {
        ...
        financials: Financials { total_cost: 0.0, total_paid: 0.0, remaining: 0.0 },
...
fn mock_table(id: &str, row_val: f64) -> TableTruth {
    let bbox = BoundingBox { x: 0.0, y: 0.0, width: 0.0, height: 0.0, page: 1 };
    ...
    TableTruth {
        ...
        schema: TableSchema {
            columns: vec![ColumnDef { 
                name: "Value".to_string(), dtype: DataType::Float64, unit: None, nullable: false, is_critical: false 
            }],
            row_count: 1,
            col_count: 1,
        },
        rows: vec![row],
        extraction_meta: ExtractionMeta { tool_version: "Test".to_string(), timestamp: "".to_string(), confidence_score: 1.0 },
        bbox,
    }
}
...
fn test_r05_exact_match() {
    ...
    let mismatches: Vec<_> = verdicts.iter()
        .filter(|v| matches!(v.violation, ViolationType::Mismatch))
        .collect();

    assert!(mismatches.is_empty(), "Exact match should have 0 mismatches");
}
...
fn test_r05_tolerance_match() {
    ...
    assert!(mismatches.is_empty(), "Diff 0.9 should be ignored (Epsilon Law)");
}
...
fn test_r05_mismatch_detected() {
    ...
    assert!(mismatches[0].message.contains("Diff: 2"), "Message should state diff");
}
